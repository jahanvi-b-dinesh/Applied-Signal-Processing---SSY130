...
void my_lms(float const * y, float const * x, float * xhat, float * e, int block_size,
	...
	*/
	...
	/*
	int n ;
      float32_t y_u[lms_taps] ;
     for ( n=0; n<block_size ; n++){
        arm_dot_prod_f32( lms_coeffs , lms_state+n, lms_taps , xhat+n ) ;
        e[n] = (x[n] - xhat[n]);
        for (int j=0; j<lms_taps; j++ )
        {
            lms_coeffs [j] +=2*lms_mu*e[n]*lms_state[n+j];
        }
	 }  
	*/	
int n;
...
for (n = 0; n < block_size; n++) 
{
    // Compute dot product using CMSIS DSP Library
    arm_dot_prod_f32(lms_coeffs, lms_state + n, lms_taps, xhat + n);
    ...
    for (int j = 0; j < lms_taps; j++) 
	{
        // Access lms_state appropriately based on the size of the array
        lms_coeffs[j] += 2 * lms_mu * e[n] * lms_state[n + j];
    }
}
*/
...
for (int n=0; n<block_size; n++){
	...
	for (int k=0; k<lms_taps; k++) {
 	lms_coeffs[k] += 2 * lms_mu * y_book[k] * e[n];
	}
 }
